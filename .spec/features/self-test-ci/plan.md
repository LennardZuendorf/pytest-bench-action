...
- [ ] **A2** `bench/test_sample_benchmark.py` — 3 deterministic stdlib benchmarks.
...
- [ ] **A3** Capture `tests/fixtures/real_results.json` from that run (trimmed to
...
- [ ] **A4** `tests/test_real_output.py` — compare real vs real (pass), real vs
  2×-scaled (fail), save strips `data`, node read correct.
  - Validate: `python -m pytest tests/test_real_output.py -v` green.
- [ ] **A5** `scripts/selftest.sh` — full pipeline harness (see tech.md).
...
- [ ] **A-checkpoint** full suite green, commit.

## Phase B — CI workflows

- [ ] **B1** `.github/workflows/ci.yml` — unit tests + selftest on push/PR.
- [ ] **B2** `.github/workflows/benchmark.yml` — dogfood `uses: ./`.
